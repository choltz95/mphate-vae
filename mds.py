...
def embed_MDS(
    ...
    if solver == "sgd":
        ...
        Y = smacof(
            X_dist, n_components=ndim, random_state=seed, init=Y_classic, metric=True, n_jobs=n_jobs
        )
    else:
        raise RuntimeError
    if how == "metric":
    ...
    Y = smacof(X_dist, n_components=ndim, random_state=seed, init=Y, metric=False, n_jobs=n_jobs)
    # re-orient to classic
    _, Y, _ = scipy.spatial.procrustes(Y_classic, Y)
    return Y